...
#define S_NUM	12		// patterns number
#define MODE_GATE	6
...
void mode_9(){	// 
		flush_mask(); mask[1]=mask[8]=1;
	const_light(mode, M_DELAY4 );
		flush_mask(); mask[2]=mask[7]=1;
	const_light(mode, M_DELAY4 );
		flush_mask(); mask[3]=mask[6]=1;
	const_light(mode, M_DELAY4 );
		flush_mask(); mask[2]=mask[7]=1;
	const_light(mode, M_DELAY4 );	
		flush_mask(); mask[1]=mask[8]=1;
	const_light(mode, M_DELAY4 );
			flush_mask(); mask[4]=mask[5]=1;
	const_light(mode, M_DELAY4 );
			flush_mask(); mask[4]=mask[5]=1;
	const_light(mode, M_DELAY4 );
}
...
inline void mode_11(){ // heart
	flush_mask(); mask[7]=mask[6]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[8]=mask[5]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[8]=mask[4]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[7]=mask[3]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[6]=mask[2]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[7]=mask[3]=1;
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[7]=mask[3]=1;
...
inline void mode_12(){
	flush_mask(); mask[1]=mask[3]=mask[7]=1;	// 1
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[6]=1;	// 2
		const_light(mode, M_DELAY4 );	
	flush_mask(); mask[1]=mask[3]=mask[5]=mask[8]=1;	// 3
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[4]=mask[7]=mask[8]=1;	// 4
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[6]=mask[8]=1;	// _\ // 5
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[5]=mask[8]=1;	// 6
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[4]=mask[8]=1;	// 7
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[6]=mask[8]=1;	// 8
		const_light(mode, M_DELAY4 );
	flush_mask(); 		mask[2]=mask[6]=mask[8]=1;	// 9
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[6]=mask[8]=1;	// center
		const_light(mode, M_DELAY4 );
	flush_mask(); 		mask[2]=mask[6]=mask[8]=1;	// 9
		const_light(mode, M_DELAY4 );	
	flush_mask(); mask[1]=mask[3]=mask[6]=mask[8]=1;	// 8
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[4]=mask[6]=mask[8]=1;	// 7
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[2]=mask[5]=mask[6]=mask[8]=1;	// 6
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[6]=mask[8]=1;	// _\ // 5
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[4]=mask[8]=1;	// 4
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[5]=mask[8]=1;	// 3
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[6]=1;	// 2
		const_light(mode, M_DELAY4 );	
	flush_mask(); mask[1]=mask[3]=mask[7]=1;	// 1
		const_light(mode, M_DELAY4 );
	flush_mask(); mask[1]=mask[3]=mask[8]=1;	// 0
		const_light(mode, M_DELAY4 );
}
